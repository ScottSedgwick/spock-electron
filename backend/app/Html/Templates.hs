module Html.Templates (pageTemplate, role) where
...
import           Text.Blaze                    (Attribute, AttributeValue, customAttribute, dataAttribute, textValue, unsafeByteStringValue)
...
import           Web.Spock                     (ActionCtxT, lazyBytes)

import           Static.Resources              (jsResources, cssResources)

----------------------------------------------------------------------------------------------
-- Page templates
pageTemplate :: MonadIO m => String -> Html -> ActionCtxT ctx m a
pageTemplate aTitle aBody = lazyBytes $ renderHtml $ docTypeHtml ! lang "en" $ do
    headerTemplate aTitle
    bodyTemplate aBody

headerTemplate :: String -> Html
headerTemplate aTitle = Text.Blaze.XHtml5.head $ do
    meta ! charset "utf-8"
...
bodyTemplate aBody = Text.Blaze.XHtml5.body $ do
    navBar
